...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        brightnessContrast = action(
            ...
            None,
            "color",
            "调节亮度和对比度",  # "Adjust brightness and contrast",
            enabled=False,
        )
    ...
    def setDirty(self):
        ...
        if self._config["auto_save"] or self.actions.saveAuto.isChecked():
            if self.output_dir is None:
                self.output_dir = QtWidgets.QFileDialog.getExistingDirectory()

            npy_file = osp.splitext(self.imagePath)[0] + ".npy"
            label_file = osp.splitext(self.imagePath)[0] + ".json"
            if self.output_dir:
                label_file_without_path = osp.basename(label_file)
                npy_file_without_path = osp.basename(npy_file)

                label_file = osp.join(self.output_dir, label_file_without_path)
                npy_file = osp.join(self.output_dir, npy_file_without_path)
            self.saveLabels(label_file)
            self.saveImage(self.dir2dir, npy_file)
            return
        self.dirty = True
...
# ==================加载图像===================
    ...
    def loadInfo(self, dc):
        ...
        self.infoListWidget.addItem(QtWidgets.QListWidgetItem(str(dc.file_meta)))
        for key in key_list:
            try:
                item = QtWidgets.QListWidgetItem(str(dc[key]))
...
# =============打开文件===============
    ...
    def openFile(self, _value=False):
        ...
        self.singe_path = osp.dirname(filename[0])
        if QT5:
            filename, _ = filename
        filename = str(filename)
        if filename:
            ...
            if self.type in ['dcm', 'DCM']:
                ...
                self.dir2dir = {}
                self.file2infor = {}
                self.dir2dir[filename] = file_path
                self.file2infor[file_path] = dc
                self.importDirImages(dir, single=True)
            elif self.type in ['ima', 'IMA']:
                dir, file_path, dc = ImaProcess.dcm2png(filename)
                self.dir2dir = {}
                self.file2infor = {}
                self.dir2dir[filename] = file_path
                self.file2infor[file_path] = dc
    ...
    def _saveFile(self, filename):
        if filename and self.saveLabels(filename):
            ...
            self.saveImage(self.dir2dir, name)

    def saveImage(self, dict, npy_name):
        # todo: dir2dir done
        for k, v in dict.items():
            self.type = k.split('.')[-1]
            if osp.basename(v).split('.')[0] == osp.basename(npy_name).split('.')[0]:
                # todo
                if self.type in ['dcm', 'DCM']:
                    DicomProcess.dcm2npy(k, npy_name)
                elif self.type in ['ima', 'IMA']:
                    ImaProcess.dcm2npy(k, npy_name)
        # print(self.dir2dir)
...
# ===================打开文件夹=======================
    ...
    def importDirImages(self, dirpath, pattern=None, load=True, single=False):

        self.statusBar().showMessage('正在处理，请等待...')
        self.actions.openNextImg.setEnabled(True)
        self.actions.openPrevImg.setEnabled(True)
        ...
        if single:
            ...
            for root, dirs, files in os.walk(dirpath):
                ...
                for file in files:
                    if file.split('.')[-1] not in ['ima', 'IMA', 'DCM', 'dcm']:
                        continue
                    self.type = file.split('.')[-1]
                    index += 1
                    self.statusBar().showMessage(
                        '正在处理:{0}/{1}'.format(index, total))
                    file_path = os.path.join(root, file)

                    if self.type in ['dcm', 'DCM']:
                        _, tmp_path, dc = DicomProcess.dcm2png(
                            file_path, single=False)
                    elif self.type in ['ima', 'IMA']:
                        _, tmp_path, dc = ImaProcess.dcm2png(
                            file_path, single=False)

                    self.dir2dir[file_path] = tmp_path
                    self.file2infor[tmp_path] = dc

        # for filename in self.scanAllImages(dirpath):
        for filename in self.scanAllImages(tmp):
        ...
        self.statusBar().showMessage('加载完毕')
