...
"""
@File    :   utils_ima.py    
@Contact :   airsimon@shu.edu.cn

@Modify Time      @Author    @Version
------------      -------    --------
2021/6/30 12:02   SimonH      1.0    
"""
...
class ImaProcess(object):
    ...
    def dcm2png(dcm, single=True):
        ...
        # ---------------------第一步：加载图像基本信息-----------------------
        key_list = ImaProcess.extra_info(dc)
        ...
        path = './tmp/'
        if single:
            ImaProcess.check_dir(path)  # 检测路径存在与否,清空文件夹
        file_path = ''
        if len(shape) == 4:  # 序列
            # todo 如果是序列的话，就直接顺序排列，不需要instance number了
            for i in range(shape[0]):
                file_path = ImaProcess.save_img(
                    shape=shape,
                    modality=modality,
    ...
    def save_img(shape, modality, dcm_array, path, patient_id, series_description, b_value, instance):
        imageData = ImaProcess.normalization(shape, dcm_array)
        file_path = path + str(patient_id) + '_' + modality + '_' + series_description + '_' + b_value + '_' + str(
            instance).zfill(4) + '.png'
